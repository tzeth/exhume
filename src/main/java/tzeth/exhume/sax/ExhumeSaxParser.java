...
public final class ExhumeSaxParser {
	...
	private class HandlerImpl extends DefaultHandler {
		...
		private void invokeStartElementHandlers(String uri, String localName, String qName, Attributes attributes) {
		...
		private Path currentPath() {
			String s = pathStack.stream().collect(Collectors.joining(Path.SEPARATOR, Path.SEPARATOR, ""));
			return Path.of(s);
		}
		
		@Override
		public void endElement(String uri, String localName, String qName) throws SAXException {
			// TODO: Distinguish between null and empty values. Must look at the "xsi:nil" attribute
		...
		private void invokeEndElementHandlers(String uri, String localName, String qName, @Nullable String value) {
	...
	private static class StartElementHandler {
		...
		public StartElementHandler(Object object, Method method) {
			...
			checkArgument(parameterTypes.length == 1 && parameterTypes[0].equals(StartOfElement.class), 
	...
	private static class EndElementHandler {
		...
		private ValueFactory getValueFactory(Class<?> type) {
			if (type.equals(EndOfElement.class)) {
				...
				throw new IllegalArgumentException("An ElementEnd handler must take one of the following types as input: " +
